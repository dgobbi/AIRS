...
//----------------------------------------------------------------------------
void vtkGaussianInterpolator::SetLabelling(int val)
{
  val = (val != 0);
  if (this->Labelling != val)
    {
    this->Labelling = val;
    this->Modified();
    }
...
//----------------------------------------------------------------------------
template <class F, class T>
void vtkImageGaussInterpolate<F, T>::Label(
  vtkInterpolationInfo *info, const F point[3], F *outPtr)
{
  const T *inPtr = static_cast<const T *>(info->Pointer);
  int *inExt = info->Extent;
  ...
  // the labels
  F labelWeights[VTK_GAUSS_KERNEL_SIZE_MAX];
  T labels[VTK_GAUSS_KERNEL_SIZE_MAX];
  int foundLabels = 0;
  ...
  do // loop over components
    ...
    do // loop over z
      ...
      do // loop over y
        ...
        do
          ...
          if (li == foundLabels && li < VTK_GAUSS_KERNEL_SIZE_MAX)
            {
            labels[li] = label;
            labelWeights[li] = val;
            foundLabels++;
    ...
    F maxweight = 0;
    T label = 0;
    for (int li = 0; li < foundLabels; li++)
      {
  ...
  int dataType, bool labelling)
{
  if (!labelling)
    ...
    switch (dataType)
      ...
      vtkTemplateAliasMacro(
        *interpolate =
          &(vtkImageGaussInterpolate<F, VTK_TT>::Label)
        );
      default:
        *interpolate = 0;
      }
  ...
  void (**func)(vtkInterpolationInfo *, const double [3], double *))
{
  vtkGaussianInterpolatorGetInterpolationFunc(
    func, this->InterpolationInfo->ScalarType, this->Labelling);
}
