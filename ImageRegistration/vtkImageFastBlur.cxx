...
{
  this->ResizeMethod = vtkImageFastBlur::OUTPUT_DIMENSIONS;
...
//----------------------------------------------------------------------------
const char *vtkImageFastBlur::GetResizeMethodAsString()
{
  switch (this->ResizeMethod)
    {
    case vtkImageFastBlur::OUTPUT_DIMENSIONS:
      return "OutputDimensions";
    case vtkImageFastBlur::OUTPUT_SPACING:
      return "OutputSpacing";
    case vtkImageFastBlur::MAGNIFICATION_FACTORS:
...
//----------------------------------------------------------------------------
int vtkImageFastBlur::RequestInformation(
...
//----------------------------------------------------------------------------
int vtkImageFastBlur::RequestUpdateExtent(
  vtkInformation *, vtkInformationVector **inputVector,
  vtkInformationVector *outputVector)
{
  vtkInformation *outInfo = outputVector->GetInformationObject(0);
...
template<class T>
void vtkImageFastBlurFilterYOrZ(
  ...
  const double *f, int kernelSize)
{
  ...
  if (kernelSize == 1)
    ...
    do
      {
      vtkImageFastBlurConvert(*tmpPtr, *outPtr);
      outPtr++;
      tmpPtr++;
      }
    while (--rowCounter);
  ...
  double **workPtr, int direction, vtkAlgorithm *progress)
{
  ...
  if (kernelSize == 1)
    ...
    for (int idY = idYMin; idY <= idYMax; idY++)
      ...
      vtkImageFastBlurFilterX(
        &inPtr[*a], *workPtr, ncomp, extent, aX, fX, kernelSizeX);

      vtkImageFastBlurFilterYOrZ(
...
int vtkImageFastBlur::RequestData(
...
//----------------------------------------------------------------------------
void vtkImageFastBlur::ThreadedRequestData(vtkInformation *,
...
//----------------------------------------------------------------------------
void vtkImageFastBlur::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
vtkAbstractImageInterpolator *vtkImageFastBlur::GetInterpolator()
{
...
//----------------------------------------------------------------------------
vtkAbstractImageInterpolator *vtkImageFastBlur::GetInternalInterpolator()
{
  if (this->Interpolate)
    {
...
//----------------------------------------------------------------------------
unsigned long int vtkImageFastBlur::GetMTime()
{
