...
vtkStandardNewMacro(vtkNIIWriter);
vtkCxxSetObjectMacro(vtkNIIWriter,QFormMatrix,vtkMatrix4x4);
vtkCxxSetObjectMacro(vtkNIIWriter,SFormMatrix,vtkMatrix4x4);
vtkCxxSetObjectMacro(vtkNIIWriter,NIFTIHeader,vtkNIIHeader);

//----------------------------------------------------------------------------
vtkNIIWriter::vtkNIIWriter()
{
...
//----------------------------------------------------------------------------
vtkNIIHeader *vtkNIIWriter::GetNIFTIHeader()
{
  if (!this->NIFTIHeader)
    {
...
//----------------------------------------------------------------------------
void vtkNIIWriter::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
char *vtkNIIWriter::ReplaceExtension(
...
void vtkNIIWriterSetInformation(
  nifti_2_header *hdr,
  vtkInformation *info)
{
...
//----------------------------------------------------------------------------
int vtkNIIWriter::GenerateHeader(vtkInformation *info, bool singleFile)
{
  // create the header
  nifti_2_header hdr;
  int version = 0;
  if (this->OwnHeader == 0)
    {
    this->OwnHeader = vtkNIIHeader::New();
    }
  else
    {
  ...
  vtkNIIWriterSetInformation(&hdr, info);
  if (hdr.datatype == 0)
    {
  ...
  hdr.sizeof_hdr = (version == 2 ?
                    vtkNIIHeader::Nifti2HeaderSize :
                    vtkNIIHeader::Nifti1HeaderSize);
  ...
  if (this->QFormMatrix ||
    ...
    double mat16[16];
    vtkNIIWriterMatrix(mat16, this->QFormMatrix, origin);
    vtkNIIWriterSetQForm(&hdr, mat16, qfac);
    }
  ...
  if (this->SFormMatrix)
    ...
    double mat16[16];
    vtkNIIWriterMatrix(mat16, this->SFormMatrix, origin);
    vtkNIIWriterSetSForm(&hdr, mat16, qfac);
    }
  ...
  vtkInformationVector* vtkNotUsed(outputVector))
{
  ...
  char *hdrname = vtkNIIWriter::ReplaceExtension(
    filename, ".img", ".hdr");
  char *imgname = vtkNIIWriter::ReplaceExtension(
    filename, ".hdr", ".img");
