...
vtkStandardNewMacro(vtkNIIReader);

//----------------------------------------------------------------------------
vtkNIIReader::vtkNIIReader()
{
  for (int i = 0; i < 8; i++)
    {
    this->Dim[i] = 0;
...
void vtkNIIReaderSwapHeader(nifti_1_header *hdr)
{
...
void vtkNIIReaderSwapHeader(nifti_2_header *hdr)
{
  vtkByteSwap::SwapVoidRange(&hdr->sizeof_hdr,    1, 4);
  vtkByteSwap::SwapVoidRange(&hdr->datatype,      1, 2);
  vtkByteSwap::SwapVoidRange(&hdr->bitpix,        1, 2);
...
//----------------------------------------------------------------------------
vtkNIIHeader *vtkNIIReader::GetNIFTIHeader()
{
  if (!this->NIFTIHeader)
    {
    this->NIFTIHeader = vtkNIIHeader::New();
    }
  return this->NIFTIHeader;
}

//----------------------------------------------------------------------------
void vtkNIIReader::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
bool vtkNIIReader::CheckExtension(
...
//----------------------------------------------------------------------------
char *vtkNIIReader::ReplaceExtension(
  const char *filename, const char *ext1, const char *ext2)
{
  ...
  if (strlen(ext1) == 4 && ext1[0] == '.' &&
    ...
    if (vtkNIIReader::CheckExtension(filename, ext1))
      {
...
//----------------------------------------------------------------------------
int vtkNIIReader::CheckNIFTIVersion(const nifti_1_header *hdr)
{
  int version = 0;
...
//----------------------------------------------------------------------------
bool vtkNIIReader::CheckAnalyzeHeader(const nifti_1_header *hdr)
{
...
//----------------------------------------------------------------------------
int vtkNIIReader::CanReadFile(const char *filename)
{
  vtkDebugMacro("Opening NIFTI file " << filename);

  char *hdrname = vtkNIIReader::ReplaceExtension(
    filename, ".img", ".hdr");
  ...
  bool canRead = false;
  nifti_1_header hdr;
  int hsize = vtkNIIHeader::Nifti1HeaderSize; // nifti_1 header size
  int rsize = gzread(file, &hdr, hsize);
  if (rsize == hsize)
    {
    int version = vtkNIIReader::CheckNIFTIVersion(&hdr);
    if (version > 0)
      {
      // NIFTI file
      canRead = true;
      ...
      {
      // Analyze 7.5 file
      canRead = vtkNIIReader::CheckAnalyzeHeader(&hdr);
      }
    }
...
//----------------------------------------------------------------------------
int vtkNIIReader::RequestInformation(
  ...
  vtkInformationVector* outputVector)
{
  ...
  if (this->FileNames)
    ...
    for (int i = 0; i < 2; i++)
      ...
      if (vtkNIIReader::CheckExtension(filename, ".hdr"))
        {
        headers++;
  ...
  if (hdrname == 0)
    {
    hdrname = vtkNIIReader::ReplaceExtension(
      filename, ".img", ".hdr");
    }
  ...
  const int hsize = vtkNIIHeader::Nifti1HeaderSize;
  int rsize = gzread(file, hdr1, hsize);
  if (rsize == hsize)
    {
    niftiVersion = vtkNIIReader::CheckNIFTIVersion(hdr1);
    if (niftiVersion >= 2)
      {
      // the header was a NIFTIv2 header
      const int h2size = vtkNIIHeader::Nifti2HeaderSize;
      ...
      {
      // Analyze 7.5 file
      canRead = vtkNIIReader::CheckAnalyzeHeader(hdr1);
      }
    }
  ...
  vtkInformationVector* outputVector)
{
  ...
  if (imgname == 0)
    {
    imgname = vtkNIIReader::ReplaceExtension(filename, ".hdr", ".img");
    }
